...
export default class Farms extends React.Component {
    ...
    fetchFarms = async () => {
        try {
            let farms = await window.$.get(`${window.config.farm_api}/api/farm-info-avax/`)
    ...
    filterByName = (name) => {
        if (!name) {
            this.setState({ filteredByName: '', filteredFarms: JSON.parse(JSON.stringify(this.state.farms)) })
            return
        }
        let filteredFarms = JSON.parse(JSON.stringify(this.state.farms))
        filteredFarms = filteredFarms.filter(farm => String(farm.pool_name).toLowerCase().startsWith(name.toLowerCase()) || String(farm.pair_name).toLowerCase().startsWith(name.toLowerCase()) )
        ...
        const columns = [
            {
                ...
                minWidth: '80px',
                right: true,
                cell: (row, i) => <div>{/*i+1*/} <img src={row.link_logo} height='40' width='40' style={{objectFit: 'cover'}} /></div>
            },
            {
                name: 'Pool',
                selector: 'pair_name',
                sortable: true,
                cell: (row) => <a className='l-clr-purple' target='_blank' rel='noopener noreferrer' href={row.link_pair}> {row.pair_name} </a>
            },
            {
                name: "Project",
                selector: 'pool_name',
            ...
            <div>
                <div className='row px-3'>
                    ...
                    <div className="search-box">
                            <form id="searchform">
                                <input value={this.state.filteredByName} onChange={e => {
                                    this.setState({filteredByName: e.target.value})
                                    this.filterByName(e.target.value)
                                }} type="text" id="search-bar" style={{paddingBottom: '3px'}} autoComplete="off" placeholder="Filter by Name" className='l-border-black' />
