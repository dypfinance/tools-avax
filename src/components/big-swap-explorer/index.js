...
export default class BigSwapExplorer extends React.Component {
    ...
    filterByTokenSymbol = (tokenSymbol) => {
        if (!tokenSymbol) {
            this.setState({ filteredByTokenSymbol: '', filteredSwaps: JSON.parse(JSON.stringify(this.state.swaps)) })
            return
        }
        let filteredSwaps = JSON.parse(JSON.stringify(this.state.swaps))
        filteredSwaps = filteredSwaps.filter(txn => String(txn.pair.token0.symbol).toLowerCase().startsWith(tokenSymbol.toLowerCase()) || String(txn.pair.token1.symbol).toLowerCase().startsWith(tokenSymbol.toLowerCase()) )
        ...
        const columns = [
            {
                ...
                sortable: true,
                // minWidth: '145px',
                cell: txn => <NavLink className='l-clr-purple' to={`/pair-explorer/${txn.pair.id}`} style={{whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden'}} title={txn.pair_symbols}>{txn.pair_symbols}</NavLink>
            },
            ...
            },
            {
                ...
                cell: txn => (
                    <div className='l-table-actions'>
                        ...
                        <NavLink title={txn.pair.id} to={`/pair-explorer/${txn.pair.id}`}><i style={{ fontSize: '20px', position: 'relative', top: '5px' }} className='far fa-compass'></i></NavLink>
                    </div>
                )
            },
            ...
            <div>
                <div className='row px-3'>
                    <div className='col-md-6'>
                        <h2 style={{display: 'block', color: `var(--preloader-clr)`}}>Big Swap Explorer</h2>
                        <p className='d-block'>Search for Big Swaps on Pangolin with useful information.</p>
                    </div>
                    <div className='col-md-6'>
