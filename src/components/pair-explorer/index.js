...
export default class PairExplorer extends React.Component {
    ...
    GetDataTable = () => {
        const columns = [
           ...
           
            {
                ...
                selector: 'maker',
                sortable: false,
                cell: txn => <a className='l-clr-purple' target="_blank" rel="noopener noreferrer" href={`https://cchain.explorer.avax.network/address/${txn.maker}`}>...{txn.maker?.slice(34)}</a>
            },
            {
                name: 'Others',
                // minWidth: '165px',
                cell: txn => (
                    <div className='l-table-actions'>
                        ...
                        <a rel='noopener noreferrer' target="_blank" title={txn.id.split('-')[0]} href={`https://cchain.explorer.avax.network/tx/${txn.id.split('-')[0]}`}><img className='icon-bg-white-rounded' src="/images/cchain.png" width="18" alt="" /></a>
    ...
    render() {
        if (!this.props.match.params.pair_id) return <Redirect to='/pair-explorer/0x497070e8b6c55fd283d8b259a6971261e2021c01' />
                        ...
                        <div className="content-title">
                            <div className="content-title-top">
                                ...
                                <ul className='l-social-icons-list'>
                                    <li>
                                        <a rel='noopener noreferrer' target="_blank" title="Buy at Pangolin" href={`https://v2.info.uniswap.org/pair/${this.props.match.params.pair_id}`}><img src="/images/pangolin.png" width="18" alt="" /></a>
                                    </li>
                                    ...
                                    <li>
                                        <a rel='noopener noreferrer' target="_blank" title={this.state.pair?.id} href={`https://cchain.explorer.avax.network/address/${this.props.match.params.pair_id}`}><img className='icon-bg-white-rounded' src="/images/cchain.png" width="18" alt="" /></a>
                                    </li>
                                    <li>
                            ...
                            <p style={{fontSize: '.8rem'}}>({this.state.mainToken?.name || '...'})Token contract: <a rel='noopener noreferrer' target='_blank' href={`https://cchain.explorer.avax.network/tokens/${this.state.mainToken?.id}`}>...{this.state.mainToken?.id.slice(34)}</a> </p>
                        </div>
                        <div className="graph-header">
                            <div className="graph-header-left">
                                <h2>${(this.state.mainToken?.derivedETH*this.state.ethPrice).toFixed(4)*1 || '...'}</h2>
                                <p>{this.state[`diffUsdPerToken${this.state.mainToken?.__number}Percent`] != '...' && <span className={this.state[`diffUsdPerToken${this.state.mainToken?.__number}Percent`]*1 >= 0 ? 'green-text':''} >(24h: {this.state[`diffUsdPerToken${this.state.mainToken?.__number}Percent`]}%)</span>} {(this.state.mainToken?.derivedETH*1).toFixed(6)*1 || '...'} AVAX</p>
                            </div>
                            <div className="graph-header-right">
                                <a target='_blank' rel='noopener noreferrer' href={`https://app.pangolin.exchange/#/swap?outputCurrency=${this.state.mainToken?.id}`}><button className="btn v1">Trade</button></a>
