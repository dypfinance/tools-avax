...
export default class TopTokens extends React.Component {
    ...
    filterByTokenSymbol = (tokenSymbol) => {
        if (!tokenSymbol) {
            this.setState({ filteredByTokenSymbol: '', filteredTokens: JSON.parse(JSON.stringify(this.state.tokens)) })
            return
        }
        let filteredTokens = JSON.parse(JSON.stringify(this.state.tokens))
        filteredTokens = filteredTokens.filter(txn => String(txn.symbol).toLowerCase().startsWith(tokenSymbol.toLowerCase()) || String(txn.name).toLowerCase().startsWith(tokenSymbol.toLowerCase()) )
        ...
        const columns = [
            {
                ...
                sortable: true,
                // minWidth: '145px',
                cell: txn => <a target='_blank' className='l-clr-purple' href={`https://cchain.explorer.avax.network/address/${txn.id}`} style={{whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}} title={txn.name}>{txn.name}</a>
            },

            {
                ...
                sortable: true,
                // minWidth: '145px',
                cell: txn => <td style={{whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}} title={txn.symbol}>{txn.symbol}</td>
            },
            {
                name: 'Liquidity',
                selector: 'liquidity',
                sortable: true,
                // minWidth: '145px',
                cell: txn => <td style={{whiteSpace: 'nowrap'}} title={txn.liquidity}>${getFormattedNumber(txn.liquidity, 2)}</td>
            },
            {
                name: 'Volume (24hrs)',
                ...
                sortable: true,
                // minWidth: '145px',
                cell: txn => <td style={{whiteSpace: 'nowrap'}} title={txn.dailyVolume}>${getFormattedNumber(txn.dailyVolume, 2)}</td>
            },
            {
                name: 'Price',
                selector: 'priceUSD',
                sortable: true,
                // minWidth: '145px',
                cell: txn => <td style={{whiteSpace: 'nowrap'}} title={txn.priceUSD}>${getFormattedNumber(txn.priceUSD, 2)}</td>
            },
            {
                ...
                sortable: true,
                // minWidth: '145px',
                cell: txn => <td className={ txn.priceChange >= 0 ? 'green-text' : 'red-text'} style={{whiteSpace: 'nowrap', }} title={txn.priceChange}>{txn.priceChange}%</td>
            },
            // {
            //     name: 'Liquidity',
            //     selector: 'liquidity',
