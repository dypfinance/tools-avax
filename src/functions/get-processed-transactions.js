export default function getProcessedTransactions(network) {
    ...
    return new Promise((resolve, reject) => {
        fetch(eval(config + '.subgraph_url') || 'https://api.thegraph.com/subgraphs/name/dasconnor/pangolin-dex', {
            method: 'POST',
            mode: 'cors',
...
function handleTheGraphData({data}) {
    ...
    let burns = data.burns.filter(mintOrBurn => {
        return [mintOrBurn.pair.token0.id, mintOrBurn.pair.token1.id].includes(eval(window.network+'.weth_address'))
